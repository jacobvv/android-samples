...
final class PermissionActivity_PermissionHelperDemo {
    ...
    static void takePhoto_WithCheck(@NonNull PermissionActivity target) {
        ...
        if (deniedForever.isEmpty()) {
            ...
            if (!denied.isEmpty()) {
                ...
                target.takePhotoRationale(request, denied);
            } else {
        ...
        switch (requestCode) {
            case REQUEST_TAKEPHOTO:
                if (deniedForever.isEmpty()) {
                    ...
                    target.takePhotoDenied(denied, deniedForever);
                }
                break;
            default:
                break;
    ...
    private static final class TakePhoto_PermissionRequest
        ...
        @Override
        public void cancel(@NonNull PermissionActivity target) {
            target.takePhotoDenied(denied, deniedForever);
        }
    }
